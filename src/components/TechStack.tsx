          ...
          {categories.map((category) => (
            <button
              ...
              className={`relative px-4 py-2 text-lg font-medium transition-colors duration-300 ${
                category.active ? "bg-gradient-to-r from-[#45BCA0] to-[#346197] bg-clip-text text-transparent" : "text-black hover:bg-gradient-to-r from-[#45BCA0] to-[#346197] hover:bg-clip-text hover:text-transparent"
              }`}
            >
              {category.name}
              {category.active && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-[#45BCA0] to-[#346197]"></div>}
            </button>
          ))}
        </div>
  
        {/* Technology Grid */}
        <div className="flex flex-wrap space-x-10 mt-10 border-b border-b-[#E7DAED] p-10 space-y-5 items-center justify-center">
          {activeTechnologies.map((tech) => (
            ...
            >
              <img src={tech.logo || "/placeholder.svg"} alt={tech.alt} className="w-40 h-14 object-contain" />
            </div>
          ))}
        </div>
      </div>
